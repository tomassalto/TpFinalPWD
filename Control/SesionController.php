...
class SesionController{
    ...
    public function __construct(){
        session_start();
        $this->setObjUsuario(new UsuarioController());
        if(isset($_SESSION["nombreUsuario"])){
            $usuarioNombre["usuarioNombre"] = $_SESSION["nombreUsuario"];
            $usuario = $this->getObjUsuario()->buscar($usuarioNombre);
    ...
    public function valida($param){
        ...
        if($arrayUsuario != null){
            if($param["usuarioPass"] == $arrayUsuario[0]->getUsuarioPass()){
                $this->setObjUsuario($arrayUsuario[0]);
                $idRoles = $this->getRol();
                $this->iniciar($param["usuarioNombre"], $idRoles);
                $resp = true;
            }
        }
        return $resp;
