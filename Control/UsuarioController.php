...
class UsuarioController
{
    ...
    private function cargarObjeto($param)
    {
        //print_r($param);
        $obj = null;
        if (array_key_exists('idUsuario', $param) and array_key_exists('usuarioNombre', $param) and array_key_exists('usuarioPass', $param) and array_key_exists('usuarioMail', $param)) {
            $obj = new Usuario();
            if (array_key_exists('usDeshabilitado', $param)) {
                $obj->setear($param["idUsuario"], $param["usuarioNombre"], $param["usuarioPass"], $param["usuarioMail"], $param["usDeshabilitado"]);
            } else {
                $obj->setear($param["idUsuario"], $param["usuarioNombre"], $param["usuarioPass"], $param["usuarioMail"], NULL);
            }
        }
        return $obj;
    }
    ...
    {
        $resp = false;
        $nombreUsuario["usuarioNombre"] = $param["usuarioNombre"];
    ...
    public function altaRol($objUs)
    {
        ...
        $usRol = new UsuarioRolController();
    ...
    public function buscar($param)
    {
        ...
        if ($param <> NULL) {
            ...
            if (isset($param['usuarioNombre']))
                $where .= " and usuarioNombre='" . $param['usuarioNombre'] . "'";
            if (isset($param['usuarioPass']))
                $where .= " and usuarioPass='" . $param['usuarioPass'] . "'";
            if (isset($param['usuarioMail']))
                $where .= " and usuarioMail='" . $param['$usuarioMail'] . "'";
        }
