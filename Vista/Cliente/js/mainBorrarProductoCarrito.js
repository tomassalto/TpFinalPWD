...
$(document).on("click", ".eliminar_producto_carrito", function () {
  ...
  var img = fila[0].children[0].children[0].src;
    var idCompraItem = fila.find("td.d-none").text().trim();
  Swal.fire({
    ...
    eliminar(idCompraItem, result.value);
...
console.log("idcompraitem: "+idCompraItem);
    $.ajax({
      type: "POST",
      url: "Accion/EliminarProductoCarrito.php",
      data: { idCompraItem: idCompraItem, ciCantidad: cantidad },
