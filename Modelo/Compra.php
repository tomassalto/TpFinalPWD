...
class Compra
{
    ...
    public function insertar()
    {
        $base = new BaseDatos();
        $resp = false;
        $consulta = "INSERT INTO compra (fechaCompra, idUsuario) VALUES (
		'" . $this->getFecha() . "',
    ...
    public function cargar()
    {
        ...
        if ($base->Iniciar()) {
            ...
            if ($res > -1) {
                if ($res > 0) {
                    $row = $base->Registro();
                    $this->setear($row['idCompra'], $row['fechaCompra'], $row['idUsuario']);
                    $resp = true;
                }
            }
        } else {
        ...
        if ($base->Iniciar()) {
            if ($base->Ejecutar($consultaCompra)) {
                ...
                while ($compra = $base->Registro()) {
                    ...
                    $objcompra->setear($compra['idCompra'], $compra['fechaCompra'], $compra['idUsuario']);
