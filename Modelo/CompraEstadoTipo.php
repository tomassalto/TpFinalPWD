...
class CompraEstadoTipo{
   ...
   public function getIdCompraEstadoTipo(){
    ...
    public function setear($id, $descripcion, $detalle){
        ...
        $this->setcompraEstadoTipoDescripcion($descripcion);
        $this->setcompraEstadoTipoDetalle($detalle);
    }
    ...
    public function cargar(){
        ...
        if ($base->Iniciar()) {
            ...
            if ($res > -1) {
                if ($res > 0) {
                    $row = $base->Registro();
                    $this->setear($row['idCompraEstadoTipo'], $row['compraEstadoTipoDescripcion'], $row['compraEstadoTipoDetalle']);
                    $resp = true;
                }
            }
        } else {
    ...
    public function insertar(){
        $resp = false;
        $base = new BaseDatos();
        $sql = "INSERT INTO compraestadotipo (compraEstadoTipoDescripcion, compraEstadoTipoDetalle) VALUES (
                '" . $this->getcompraEstadoTipoDescripcion() . "',
                '" . $this->getcompraEstadoTipoDetalle() . "');";
        ...
        if ($res > -1) {
            if ($res > 0) {
                ...
                while ($row = $base->Registro()) {
                    ...
                    $obj->setear($row['idCompraEstadoTipo'], $row['compraEstadoTipoDescripcion'], $row['compraEstadoTipoDetalle']);
